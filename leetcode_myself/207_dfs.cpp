...
class Solution
{
  public:
	...
	//true:有环
	//false:无环
	bool dfs(int current, vector<int> &courseStatus, const vector<vector<int>> &graph)
	{
		if (courseStatus[current] == 1)
			return true; //环

		if (courseStatus[current] == 2)
			return false; //无环

		courseStatus[current] = 1;
		for (const auto it = graph[current].begin(); it != graph[current].end(); it++)
		{
			if (dfs(*it, courseStatus, graph))
				return true;
		}
	...
	bool canFinish_map(int numCourses, vector<pair<int, int>> &prerequisites)
	{
		...
		for (int i = 0; i < numCourses; i++)
		{
			if (dfs_map(i, courseStatus, graph))
				return false;
		}
		return true;
	}
	...
	//true:有环
	//false:无环
	bool dfs_map(int current, vector<int> &courseStatus, multimap<int, int> &graph)
	{
		if (courseStatus[current] == 1)
			return true; //环
		...
		{
			if (dfs_map(it->second, courseStatus, graph))
				return true;
			it++;
		}
		courseStatus[current] = 2;
